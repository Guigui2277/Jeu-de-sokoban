...
#define COULEUR_BLEU_CLAIR "\033[1;34m"
...
/**
* @brief affiche l'entête de l'introduction
* @return résultat : affichage
*/

void introduction(){
	printf("%s##########################################%s\n", COULEUR_JAUNE, COULEUR_RESET);
	printf("%s#   BIENVENUE DANS NOTRE JEU DE SOKOBAN  #%s\n", COULEUR_JAUNE, COULEUR_RESET);
	printf("%s##########################################%s\n", COULEUR_JAUNE, COULEUR_RESET);
}
...
void afficher_entete(t_partie *jeu, char fichier[]){
	...
	printf("\033[H\033[J");
	printf("%s################################%s\n", COULEUR_JAUNE, COULEUR_RESET);
	printf("%s#        JEU DE SOKOBAN        #%s\n", COULEUR_JAUNE, COULEUR_RESET);
	printf("%s################################%s\n\n", COULEUR_JAUNE, COULEUR_RESET);
	printf("%s################################%s\n\n", COULEUR_JAUNE, COULEUR_RESET);
	printf(" %sNom de la partie : %s%s\n\n",COULEUR_BLEU_CLAIR, COULEUR_RESET, fichier);
	printf(" %sHaut : z\n Bas : s\n Gauche : q\n Droite : d%s\n", COULEUR_BLEU_CLAIR, COULEUR_RESET);
	printf(" %sPour abandonner la partie : x\n Pour continuer la partie : r%s\n", COULEUR_BLEU_CLAIR, COULEUR_RESET);
	printf(" %sPour annuler un déplacement : u%s\n", COULEUR_BLEU_CLAIR, COULEUR_RESET);
	printf(" %sPour agrandir le plateau : +\n Pour le rétrécir : -%s\n\n", COULEUR_BLEU_CLAIR, COULEUR_RESET);
	printf(" %sNombre de déplacement : %d%s\n\n", COULEUR_BLEU_CLAIR, jeu->nbDep, COULEUR_RESET);
	printf("%s################################%s\n\n", COULEUR_JAUNE, COULEUR_RESET);
}
    ...
    for (int lig=0; lig < MAXLIG; lig++) {
        for (int ligchar=0; ligchar < jeu->echelle; ligchar++) {
            for (int col=0; col < MAXLIG; col++) {
                ...
                // 1. D'ABORD : On met des valeurs par défaut pour tout le monde
                affiche = caractere;       
                couleur = COULEUR_RESET;   

                // 2. ENSUITE : On change couleur et symbole selon le cas
                if (caractere == MUR) {
                    couleur = COULEUR_BLEU;
                }
                else if (caractere == JOUEUR) {
                    couleur = COULEUR_ROUGE;
                }
                else if (caractere == CAISSE) {
                    couleur = COULEUR_JAUNE;
                }
                else if (caractere == CIBLE) {
                    couleur = COULEUR_MAGENTA;
                }
                else if (caractere == CAISSE_CIBLE) {
                    affiche = CAISSE;       // On remplace '*' par '$'
                    couleur = COULEUR_VERT; // Caisse validée en vert !
                }
                else if (caractere == JOUEUR_CIBLE) {
                    affiche = JOUEUR;       // On remplace '+' par '@'
                    couleur = COULEUR_ROUGE;
                }

                // 3. ENFIN : On affiche (Couleur -> Lettre -> Reset)
                printf("%s", couleur); // Active la couleur
                for (int colchar=0; colchar < jeu->echelle; colchar++) {
                        printf("%c", affiche); 
                }
                printf("%s", COULEUR_RESET); // Désactive la couleur
            }   
            printf("\n"); 
        }
    }
